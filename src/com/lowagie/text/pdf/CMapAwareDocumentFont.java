...
public class CMapAwareDocumentFont extends DocumentFont {
	...
	/**
	 * The CMap constructed from the ToUnicode map from the font's dictionary, if present. This CMap
	 * transforms CID values into unicode equivalent
	 */
	private CMap			toUnicodeCmap;
	/**
	 * Mapping between CID code (single byte only for now) and unicode equivalent as derived by the
	 * font's encoding. Only needed if the ToUnicode CMap is not provided.
	 */
	...
	/**
	 * Inverts DocumentFont's uni2byte mapping to obtain a cid-to-unicode mapping based on the
	 * font's encoding
	 * 
	 * @since 2.1.7
	 */
	private void processUni2Byte() {
		...
		for (int element : e) {
			...
			// this is messy, messy - an encoding can have multiple unicode values mapping to the
			// same cid - we are going to arbitrarily choose the first one
			// what we really need to do is to parse the encoding, and handle the differences info
			// ourselves. This is a huge duplication of code of what is already
			// being done in DocumentFont, so I really hate to go down that path without seriously
			// thinking about a change in the organization of the Font class hierarchy
			if (cidbyte2uni[n] == 0) {
	...
	/**
	 * For all widths of all glyphs, compute the average width in normalized 1000 point units. This
	 * is used to give some meaningful width in cases where we need an average font width (such as
	 * if the width of a space isn't specified by a given font)
	 * 
	...
	/**
	 * @since 2.1.5 Override to allow special handling for fonts that don't specify width of space
	 *        character
