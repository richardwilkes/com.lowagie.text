...
public abstract class BaseFont {
	...
	/**
	 * The maximum height above the baseline reached by glyphs in this font, excluding the height of
	...
	/**
	 * The maximum depth below the baseline reached by glyphs in this font. The value is a negative
	 * number.
	 */
	public static final int			DESCENT					= 3;
	/**
	 * The angle, expressed in degrees counterclockwise from the vertical, of the dominant vertical
	 * strokes of the font. The value is negative for fonts that slope to the right, as almost all
	 * italic fonts do.
	 */
	...
	/**
	 * The recommended vertical offset from the baseline for subscripts for this font. Usually a
	 * negative value.
	 */
	...
	/**
	 * true if the font must use its built in encoding. In that case the <CODE>encoding</CODE> is
	 * only used to map a char to the position inside the font, not to the expected char name.
	 */
	...
	static class StreamFont extends PdfStream {
		 ...
		 * @param lengths an array of int that describes the several lengths of each part of the
		 *            font
	...
	/**
	 * Creates a new font. This will always be the default Helvetica font (not embedded). This
	 * method is introduced because Helvetica is used in many examples.
	 * 
	...
	/**
	 * Creates a new font. This font can be one of the 14 built in types, a Type1 font referred to
	 * by an AFM or PFM file, a TrueType font (simple or collection) or a CJK font from the Adobe
	 * Asian Font Pack. TrueType fonts and CJK fonts can have an optional style modifier appended to
	 * the name. These modifiers are: Bold, Italic and BoldItalic. An example would be
	 * "STSong-Light,Bold". Note that this modifiers do not work if the font is embedded. Fonts in
	 * TrueType collections are addressed by index such as "msgothic.ttc,1". This would get the
	 * second font (indexes start at 0), in this case "MS PGothic".
	 * <P>
	 * The fonts are cached and if they already exist they are extracted from the cache, not parsed
	 * again.
	 * <P>
	 * Besides the common encodings described by name, custom encodings can also be made. These
	 * encodings will only work for the single byte fonts Type1 and TrueType. The encoding string
	 * starts with a '#' followed by "simple" or "full". If "simple" there is a decimal for the
	 * first character position and then a list of hex values representing the Unicode codes that
	 * compose that encoding.<br>
	 * The "simple" encoding is recommended for TrueType fonts as the "full" encoding risks not
	 * matching the character with the right glyph if not done with care.<br>
	 * The "full" encoding is specially aimed at Type1 fonts where the glyphs have to be described
	 * by non standard names like the Tex math fonts. Each group of three elements compose a code
	 * position: the one byte code order in decimal or as 'x' (x cannot be the space), the name and
	 * the Unicode character used to access the glyph. The space must be assigned to character
	 * position 32 otherwise text justification will not work.
	 * <P>
	 * Example for a "simple" encoding that includes the Unicode character space, A, B and
	 * ecyrillic:
	 * 
	 * <PRE>
	 ...
	 * @param encoding the encoding to be applied to this font
	 * @param embedded true if the font is to be embedded in the PDF
	 * @param forceRead in some cases (TrueTypeFont, Type1Font), the full font file will be read and
	 *            kept in memory if forceRead is true
	 * @return returns a new font. This font may come from the cache
	 * @throws DocumentException the font is invalid
	 * @throws IOException the font file could not be read
	 * @since 2.1.5
	 ...
	 * second font (indexes start at 0), in this case "MS PGothic".
	 * <P>
	 * The fonts may or may not be cached depending on the flag <CODE>cached</CODE>. If the
	 * <CODE>byte</CODE> arrays are present the font will be read from them instead of the name. A
	 * name is still required to identify the font type.
	 * <P>
	 * Besides the common encodings described by name, custom encodings can also be made. These
	 * encodings will only work for the single byte fonts Type1 and TrueType. The encoding string
	 * starts with a '#' followed by "simple" or "full". If "simple" there is a decimal for the
	 ...
	 * @param encoding the encoding to be applied to this font
	 * @param embedded true if the font is to be embedded in the PDF
	 * @param cached true if the font comes from the cache or is added to the cache if new, false if
	 *            the font is always created new
	 * @param ttfAfm the true type font or the afm in a byte array
	 * @param pfb the pfb in a byte array
	 * @return returns a new font. This font may come from the cache but only if cached is true,
	 *         otherwise it will always be created new
	 * @throws DocumentException the font is invalid
	 * @throws IOException the font file could not be read
	 * @since iText 0.80
	 */
	 ...
	 * @param ttfAfm the true type font or the afm in a byte array
	 * @param pfb the pfb in a byte array
	 * @param noThrow if true will not throw an exception if the font is not recognized and will
	 *            return null, if false will throw an exception if the font is not recognized. Note
	 *            that even if true an exception may be thrown in some circumstances. This parameter
	 *            is useful for FontFactory that may have to check many invalid font names before
	...
	/**
	 * Creates a font based on an existing document font. The created font font may not behave as
	 * expected, depending on the encoding or subset.
	 * 
	...
	/**
	 * Normalize the encoding names. "winansi" is changed to "Cp1252" and "macroman" is changed to
	 * "MacRoman".
	 * 
	...
	/**
	 * Gets the width from the font according to the Unicode char <CODE>c</CODE> or the
	 * <CODE>name</CODE>. If the <CODE>name</CODE> is null it's a symbolic font.
	 * 
	...
	/**
	 * Gets the descent of a <CODE>String</CODE> in normalized 1000 units. The descent will always
	 * be less than or equal to zero even if all the characters have an higher descent.
	 * 
	...
	/**
	 * Gets the descent of a <CODE>String</CODE> in points. The descent will always be less than or
	 * equal to zero even if all the characters have an higher descent.
	 * 
	 * @param text the <CODE>String</CODE> to get the descent of
	...
	/**
	 * Gets the ascent of a <CODE>String</CODE> in points. The ascent will always be greater than or
	...
	/**
	 * Returns a PdfStream object with the full font program (if possible). This method will return
	 * null for some types of fonts (CJKFont, Type3Font) or if there is no font program available
	 ...
	 * <CODE>ASCENT</CODE>, <CODE>AWT_ASCENT</CODE>, <CODE>CAPHEIGHT</CODE>, <CODE>DESCENT</CODE>,
	 * <CODE>AWT_DESCENT</CODE>, <CODE>ITALICANGLE</CODE>, <CODE>BBOXLLX</CODE>,
	 * <CODE>BBOXLLY</CODE>, <CODE>BBOXURX</CODE> and <CODE>BBOXURY</CODE>.
	 * 
	...
	/**
	 * Gets the font type. The font types can be: FONT_TYPE_T1, FONT_TYPE_TT, FONT_TYPE_CJK and
	 * FONT_TYPE_TTUNI.
	 * 
	...
	/**
	 * Creates a unique subset prefix to be added to the font name when the font is embedded and
	 * subset.
	 * 
	...
	/**
	 * Sets the font name that will appear in the pdf font dictionary. Use with care as it can
	 * easily make a font unreadable if not embedded.
	 * 
	...
	/**
	 * Gets the full name of the font. If it is a True Type font each array element will have
	 * {Platform ID, Platform Encoding ID, Language ID, font name}. The interpretation of this
	 * values can be found in the Open Type specification, chapter 2, in the 'name' table.<br>
	...
	/**
	 * Gets all the entries of the names-table. If it is a True Type font each array element will
	 * have {Name ID, Platform ID, Platform Encoding ID, Language ID, font name}. The interpretation
	 * of this values can be found in the Open Type specification, chapter 2, in the 'name'
	 * table.<br>
	 ...
	 * @throws DocumentException on error
	 * @throws IOException on error
	 * @return an array of Object[] built with {getPostscriptFontName(), getFamilyFontName(),
	 *         getFullFontName()}
	 */
	...
	/**
	 * Gets the family name of the font. If it is a True Type font each array element will have
	 * {Platform ID, Platform Encoding ID, Language ID, font name}. The interpretation of this
	 * values can be found in the Open Type specification, chapter 2, in the 'name' table.<br>
	 * For the other fonts the array has a single element with {"", "", "", font name}.
	 * 
	...
	/**
	 * Sets the conversion of <CODE>char</CODE> directly to <CODE>byte</CODE> by casting. This is a
	 * low level feature to put the bytes directly in the content stream without passing through
	 * String.getBytes().
	 * 
	...
	/**
	 * Indicates if all the glyphs and widths for that particular encoding should be included in the
	 * document. When set to <CODE>true</CODE> only the glyphs used will be included in the font.
	 * When set to <CODE>false</CODE> and {@link #addSubsetRange(int[])} was not called the full
	 * font will be included otherwise just the characters ranges will be included.
	 * 
	 ...
	 * @return the <CODE>InputStream</CODE> to get the resource or <CODE>null</CODE> if not found
	 */
	 ...
	 * has only meaning with CJK fonts with Identity encoding.
	 * 
	 ...
	 * @return <CODE>true</CODE> if the character has a glyph, <CODE>false</CODE> otherwise
	 */
	 ...
	 * @return <CODE>true</CODE> if the advance was set, <CODE>false</CODE> otherwise
	 */
	...
	/**
	 * Gets a list of all document fonts. Each element of the <CODE>ArrayList</CODE> contains a
	 * <CODE>Object[]{String,PRIndirectReference}</CODE> with the font name and the indirect
	 * reference to it.
	 * 
	 ...
	 * <CODE>ArrayList</CODE> contains a <CODE>Object[]{String,PRIndirectReference}</CODE> with the
	 * font name and the indirect reference to it.
	 * 
	 * @param reader the document where the fonts are to be listed from
	...
	/**
	 * Gets the smallest box enclosing the character contours. It will return <CODE>null</CODE> if
	 * the font has not the information or the character has no contours, as in the case of the
	 * space, for example. Characters with no contours may also return [0,0,0,0].
	 * 
	...
	/**
	 * iText expects Arabic Diactrics (tashkeel) to have zero advance but some fonts, most notably
	 * those that come with Windows, like times.ttf, have non-zero advance for those characters.
	 * This method makes those character to have zero width advance and work correctly in the iText
	 * Arabic shaping and reordering context.
	 */
	...
	/**
	 * Adds a character range when subsetting. The range is an <CODE>int</CODE> array where the
	 * first element is the start range inclusive and the second element is the end range inclusive.
