...
/**
 * <CODE>PdfDocument</CODE> is the class that is used by <CODE>PdfWriter</CODE> to translate a
 * <CODE>Document</CODE> into a PDF with different pages.
 * <P>
...
public class PdfDocument extends Document {
	 ...
	 * location other than the first page should be shown when the document is opened.<BR>
	...
	/**
	 * Closes the document. <B> Once all the content has been written in the body, you have to close
	...
	public boolean newPage() {
		...
		try {
			...
			if (transition != null) {
				page.put(PdfName.TRANS, transition.getTransitionDictionary());
				transition = null;
			}
			if (duration > 0) {
	 ...
	 *         destination with the same name already existed
	 */
