...
public class AcroFields {
	...
	/**
	 * Gets the list of appearance names. Use it to get the names allowed with radio and checkbox
	 * fields. If the /Opt key exists the values will also be included. The name 'Off' may also be
	 * valid even if not returned in the list.
	 *
	...
	/**
	 * Gets the list of export option values from fields of type list or combo. If the field doesn't
	 * exist or the field type is not list or combo it will return <CODE>null</CODE>.
	 *
	...
	/**
	 * Gets the list of display option values from fields of type list or combo. If the field
	 * doesn't exist or the field type is not list or combo it will return <CODE>null</CODE>.
	 *
	 * @param fieldName the field name
	 * @return the list of export option values from fields of type list or combo
	 */
	 ...
	 * <CODE>displayValues</CODE> may be <CODE>null</CODE> but not both. This method will only set
	 * the list but will not set the value or appearance. For that, calling <CODE>setField()</CODE>
	 * is required.
	 * <p>
	 * An example:
	 * <p>
	 ...
	 * <CODE>FIELD_TYPE_TEXT</CODE>, <CODE>FIELD_TYPE_LIST</CODE>, <CODE>FIELD_TYPE_COMBO</CODE> or
	 * <CODE>FIELD_TYPE_SIGNATURE</CODE>.
	 * <p>
	 * If the field does not exist or is invalid it returns <CODE>FIELD_TYPE_NONE</CODE>.
	 *
	 * @param fieldName the field name
	...
	/**
	 * Renames a field. Only the last part of the name can be renamed. For example, if the original
	 * field is "ab.cd.ef" only the "ef" part can be renamed.
	 *
	 ...
	 * <CODE>java.awt.Color</CODE>. If <code>null</code> removes the background.<br>
	 ...
	 * @param inst an array of <CODE>int</CODE> indexing into <CODE>AcroField.Item.merged</CODE>
	 *            elements to process. Set to <CODE>null</CODE> to process all
	 ...
	 * <p>
	 * <ul>
	 * <li>flags - a set of flags specifying various characteristics of the field's widget
	 * annotation. The value of this entry replaces that of the F entry in the form's corresponding
	 * annotation dictionary.<br>
	 * <li>setflags - a set of flags to be set (turned on) in the F entry of the form's
	 * corresponding widget annotation dictionary. Bits equal to 1 cause the corresponding bits in F
	 * to be set to 1.<br>
	 * <li>clrflags - a set of flags to be cleared (turned off) in the F entry of the form's
	 * corresponding widget annotation dictionary. Bits equal to 1 cause the corresponding bits in F
	 * to be set to 0.<br>
	 * <li>fflags - a set of flags specifying various characteristics of the field. The value of
	 * this entry replaces that of the Ff entry in the form's corresponding field dictionary.<br>
	 * <li>setfflags - a set of flags to be set (turned on) in the Ff entry of the form's
	 * corresponding field dictionary. Bits equal to 1 cause the corresponding bits in Ff to be set
	 * to 1.<br>
	 * <li>clrfflags - a set of flags to be cleared (turned off) in the Ff entry of the form's
	 * corresponding field dictionary. Bits equal to 1 cause the corresponding bits in Ff to be set
	 * to 0.<br>
	 * </ul>
	 *
	...
	/**
	 * Regenerates the field appearance. This is useful when you change a field property, but not
	 * its value, for instance form.setFieldProperty("f", "bgcolor", Color.BLUE, null); This won't
	 * have any effect, unless you use regenerateField("f") after changing the property.
	 *
	 * @param name the fully qualified field name or the partial name in the case of XFA forms
	 * @throws IOException on error
	 * @throws DocumentException on error
	 * @return <CODE>true</CODE> if the field was found and changed, <CODE>false</CODE> otherwise
	 */
	...
	/**
	 * Sets the field value and the display string. The display string is used to build the
	 * appearance in the cases where the value is modified by Acrobat with JavaScript and the
	 * algorithm is known.
	 *
	 * @param name the fully qualified field name or the partial name in the case of XFA forms
	 * @param value the field value
	 * @param display the string that is used for the appearance. If <CODE>null</CODE> the
	 *            <CODE>value</CODE> parameter will be used
	 * @return <CODE>true</CODE> if the field was found and changed, <CODE>false</CODE> otherwise
	 * @throws IOException on error
	 * @throws DocumentException on error
	 */
	...
	/**
	 * Sets different values in a list selection. No appearance is generated yet; nor does the code
	...
	/**
	 * Gets all the fields. The fields are keyed by the fully qualified field name and the value is
	 * an instance of <CODE>AcroFields.Item</CODE>.
	 *
	 ...
	 * multiple of 5. For each of this groups the values are: [page, llx, lly, urx, ury]. The
	 * coordinates have the page rotation in consideration.
	 *
	 ...
	 * <CODE>name</CODE> are removed from the document otherwise only the fields in that particular
	 * page are removed.
	 *
	 * @param name the field name
	...
	/**
	 * Sets the option to generate appearances. Not generating appearances will speed-up form
	 * filling but the results can be unexpected in Acrobat. Don't use it unless your environment is
	...
	public static class Item {
		...
		/**
		 * This function writes the given key/value pair to all the instances of merged, widget,
		 * and/or value, depending on the <code>writeFlags</code> setting
		 *
		 * @since 2.1.5
		...
		/**
		 * Remove the given instance from this item. It is possible to remove all instances using
		 * this function.
		 *
		 * @since 2.1.5
		 * @param killIdx
		...
		/**
		 * Retrieve the merged dictionary for the given instance. The merged dictionary contains all
		 * the keys present in parent fields, though they may have been overwritten (or modified?)
		 * by children. Example: a merged radio field dict will contain /V
		 *
		 * @since 2.1.5
		 * @param idx instance index
	 ...
	 *
	 * @param name the field name
	 * @return the signature dictionary keyed by /V or <CODE>null</CODE> if the field is not a
	 *         signature
	 */
	 ...
	 * @return <CODE>true</CODE> if the signature covers the entire document, <CODE>false</CODE>
	 *         otherwise
	 */
	 ...
	 *
	 * @param field the signature field name
	 * @return an <CODE>InputStream</CODE> covering the revision. Returns <CODE>null</CODE> if it's
	...
	/**
	 * Sets a cache for field appearances. Parsing the existing PDF to create a new TextField is
	 * time expensive. For those tasks that repeatedly fill the same PDF with different field values
	 * the use of the cache has dramatic speed advantages. An example usage:
	 * <p>
	 *
	 * <pre>
	...
	/**
	 * Gets the list of substitution fonts. The list is composed of <CODE>BaseFont</CODE> and can be
	 * <CODE>null</CODE>. The fonts in this list will be used if the original font doesn't contain
	 * the needed glyphs.
	 *
	 * @return the list
	 */
	...
	/**
	 * Sets a list of substitution fonts. The list is composed of <CODE>BaseFont</CODE> and can also
	 * be <CODE>null</CODE>. The fonts in this list will be used if the original font doesn't
	 * contain the needed glyphs.
	 *
	...
	/**
	 * Creates a new pushbutton from an existing field. If there are several pushbuttons with the
	 * same name only the first one is used. This pushbutton can be changed and be used to replace
	 * an existing one, with the same name or other name, as long is it is in the same document. To
	 * replace an existing pushbutton call {@link #replacePushbuttonField(String,PdfFormField)}.
	 *
	...
	/**
	 * Creates a new pushbutton from an existing field. This pushbutton can be changed and be used
	 * to replace an existing one, with the same name or other name, as long is it is in the same
	 * document. To replace an existing pushbutton call
	 * {@link #replacePushbuttonField(String,PdfFormField,int)}.
	 *
	 * @param field the field name that should be a pushbutton
	 ...
	 * {@link #getNewPushbuttonFromField(String)} from the same document or it can be a generic
	 * PdfFormField of the type pushbutton.
	 *
	 * @param field the field name
	 * @param button the <CODE>PdfFormField</CODE> representing the pushbutton
	 * @return <CODE>true</CODE> if the field was replaced, <CODE>false</CODE> if the field was not
	 *         a pushbutton
	 */
	 ...
	 * {@link #getNewPushbuttonFromField(String,int)} from the same document or it can be a generic
	 * PdfFormField of the type pushbutton.
	 *
	 * @param field the field name
	 * @param button the <CODE>PdfFormField</CODE> representing the pushbutton
	...
	public boolean replacePushbuttonField(String field, PdfFormField button, int order) {
		...
		for (PdfName element : buttonRemove) {
