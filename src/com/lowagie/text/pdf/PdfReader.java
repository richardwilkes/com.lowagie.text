...
public class PdfReader implements PdfViewerPreferences {
	 ...
	 * Reads and parses a PDF document.
	 *
	 * @param is the <CODE>InputStream</CODE> containing the document. The stream is read to the end
	 *            but is not closed
	 * @param ownerPassword the password to read the document
	 * @throws IOException on error
	 */
	 ...
	 * into memory. The reader is said to be working in "partial" mode as only parts of the pdf are
	 * read as needed. The pdf is left open but may be closed at any time with
	...
	/**
	 * Returns the document's catalog. This dictionary is not a copy, any changes will be reflected
	 * in the catalog.
	 *
	...
	/**
	 * Gets the page size, taking rotation into account. This is a <CODE>Rectangle</CODE> with the
	 * value of the /MediaBox and the /Rotate key.
	 *
	 * @param index the page number. The first page is 1
	...
	/**
	 * Gets the page size without taking rotation into account. This is the value of the /MediaBox
	 * key.
	 *
	 * @param index the page number. The first page is 1
	...
	/**
	 * Gets the crop box without taking rotation into account. This is the value of the /CropBox
	 * key. The crop box is the part of the document to be displayed or printed. It usually is the
	 * same as the media box but may be smaller. If the page doesn't have a crop box the page size
	 * will be returned.
	 *
	 * @param index the page number. The first page is 1
	 * @return the crop box
	...
	/**
	 * Returns the content of the document information dictionary as a <CODE>HashMap</CODE> of
	 * <CODE>String</CODE>.
	 *
	...
	/**
	 * Reads a <CODE>PdfObject</CODE> resolving an indirect reference if needed. If the reader was
	 * opened in partial mode the object will be released to save memory.
	 *
	 * @param obj the <CODE>PdfObject</CODE> to read
	 * @param parent
	 * @return a PdfObject
	...
	/**
	 * Eliminates the reference to the object freeing the memory used by it and clearing the xref
	 * entry.
	 *
			...
			for (int k = start; k < end; ++k) {
				...
				if (tokens.getStringValue().equals("n")) {
					if (xref[p] == 0 && xref[p + 1] == 0) {
						// if (pos == 0)
						// tokens.throwError("File position 0 cross-reference entry in this xref
						// subsection");
						xref[p] = pos;
					}
	 ...
	 *
	 * @return <CODE>true</CODE> if the document was changed, <CODE>false</CODE> otherwise
	 */
	...
	/**
	 * Gets the PDF version. Only the last version char is returned. For example version 1.4 is
	 * returned as '4'.
	 *
	...
	/**
	 * Gets all the named destinations as an <CODE>HashMap</CODE>. The key is the name and the value
	 * is the destinations array.
	 *
	...
	/**
	 * Gets the named destinations from the /Dests key in the catalog as an <CODE>HashMap</CODE>.
	...
	/**
	 * Gets the named destinations from the /Names key in the catalog as an <CODE>HashMap</CODE>.
	 * The key is the name and the value is the destinations array.
	 *
	 * @return gets the named destinations
	 */
	...
	public String getJavaScript(RandomAccessFileOrArray file) throws IOException {
		...
		for (String sortedName : sortedNames) {
			PdfDictionary j = (PdfDictionary) getPdfObjectRelease((PdfIndirectReference) jscript.get(sortedName));
			if (j == null) {
				continue;
			}
	...
	/**
	 * Selects the pages to keep in the document. The pages are described as ranges. The page
	 * ordering can be changed but no page repetitions are allowed. Note that it may be very slow in
	 * partial mode.
	 *
	...
	/**
	 * Selects the pages to keep in the document. The pages are described as a <CODE>List</CODE> of
	 * <CODE>Integer</CODE>. The page ordering can be changed but no page repetitions are allowed.
	 * Note that it may be very slow in partial mode.
	 *
	...
	/**
	 * Returns a bitset representing the PageMode and PageLayout viewer preferences. Doesn't return
	 * any information about the ViewerPreferences dictionary.
	 *
	 ...
	 * PDF modification with iText will invalidate them. Invalidated usage rights may confuse
	 * Acrobat and it's advisable to remove them altogether.
	 */
	...
	/**
	 * Checks if the document was opened with the owner password so that the end application can
	 * decide what level of access restrictions to apply. If the document is not encrypted it will
	 * return <CODE>true</CODE>.
	 *
	 * @return <CODE>true</CODE> if the document was opened with the owner password or if it's not
	 *         encrypted, <CODE>false</CODE> if the document was opened with the user password
	 */
