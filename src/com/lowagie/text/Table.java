...
/**
 * A <CODE>Table</CODE> is a <CODE>Rectangle</CODE> that contains <CODE>Cell</CODE>s, ordered in
 * some kind of matrix.
 * <P>
 * Tables that span multiple pages are cut into different parts automatically. If you want a table
 * header to be repeated on every page, you may not forget to mark the end of the header section by
 * using the method <CODE>endHeaders()</CODE>.
 * <P>
 * The matrix of a table is not necessarily an m x n-matrix. It can contain holes or cells that are
 * bigger than the unit. Believe me or not, but it took some serious thinking to make this as user
 * friendly as possible. I hope you will find the result quite simple (I love simple solutions,
 ...
 * <TH ALIGN="Left" COLSPAN="3" VALIGN="Left">header</TH>
 * </TR>
 * <TR ALIGN="Left" VALIGN="Left">
 * <TD ALIGN="Left" BORDERCOLOR="#ff0000" ROWSPAN="2" VALIGN="Left">example cell with colspan 1 and
 * rowspan 2</TD>
 * <TD ALIGN="Left" VALIGN="Left">1.1</TD>
 * <TD ALIGN="Left" VALIGN="Left">2.1</TD>
 * </TR>
 * <TR ALIGN="Left" VALIGN="Left">
 * <TD ALIGN="Left" VALIGN="Left">1.2</TD>
 * <TD ALIGN="Left" VALIGN="Left">2.2</TD>
 * </TR>
 * <TR ALIGN="Left" VALIGN="Left">
 * <TD ALIGN="Left" VALIGN="Left">cell test1</TD>
 * <TD ALIGN="Left" COLSPAN="2" ROWSPAN="2" VALIGN="Left">big cell</TD>
 * </TR>
 * <TR ALIGN="Left" VALIGN="Left">
 * <TD ALIGN="Left" VALIGN="Left">cell test2</TD>
 * </TR>
 * </TABLE>
 *
 * @see Rectangle
...
public class Table extends Rectangle implements LargeElement {
	...
	/**
	 * This Empty Cell contains the DEFAULT layout of each Cell added with the method addCell(String
	 * content).
	 */
	...
	/**
	 * Boolean to automatically fill empty cells before a table is rendered (takes CPU so may be set
	 * to false in case of certainty)
	 */
	...
	/**
	 * Constructs a <CODE>Table</CODE> with a certain number of columns and a certain number of
	 * <CODE>Row</CODE>s.
	 *
	 * @param columns The number of columns in the table
	...
	public Table(Table t) {
		super(0, 0, 0, 0);
		cloneNonPositionParameters(t);
		columns = t.columns;
		rows = t.rows;
		curPosition = t.curPosition;
		defaultCell = t.defaultCell;
		lastHeaderRow = t.lastHeaderRow;
		alignment = t.alignment;
		cellpadding = t.cellpadding;
		cellspacing = t.cellspacing;
		width = t.width;
		widths = t.widths;
		autoFillEmptyCells = t.autoFillEmptyCells;
		tableFitsPage = t.tableFitsPage;
		cellsFitPage = t.cellsFitPage;
		offset = t.offset;
		convert2pdfptable = t.convert2pdfptable;
	}
	...
	/**
	 * Enables/disables automatic insertion of empty cells before table is rendered. (default =
	 * false) As some people may want to create a table, fill only a couple of the cells and don't
	 * bother with investigating which empty ones need to be added, this default behavior may be
	 * very welcome. Disabling is recommended to increase speed. (empty cells should be added
	 * through extra code then)
	 *
	 ...
	 * <P>
	 * You can give up relative values of borderwidths. The sum of these values will be considered
	 ...
	 * <P>
	 * When a table doesn't fit a page, it is split in two parts. If you want to avoid this, you
	 * should set the <VAR>tableFitsPage</VAR> value to true.
	 *
	 ...
	 * Allows you to control when a page break occurs.
	 * <P>
	 * When a cell doesn't fit a page, it is split in two parts. If you want to avoid this, you
	 * should set the <VAR>cellsFitPage</VAR> value to true.
	 *
	 * @param fitPage enter true if you don't want to split cells
	 */
	...
	/**
	 * Sets the offset of this table. Normally a newline is added before you add a Table object.
	 * This newline uses the current leading. If you want to control the space between the table and
	 * the previous element yourself, you have to set the offset of this table.
	 *
	 ...
	 * Adds a <CODE>Cell</CODE> to the <CODE>Table</CODE>.
	 * <P>
	 * This is a shortcut for <CODE>addCell(Cell cell)</CODE>. The <CODE>Phrase</CODE> will be
	 * converted to a <CODE>Cell</CODE>.
	 *
	 ...
	 * Adds a <CODE>Cell</CODE> to the <CODE>Table</CODE>.
	 * <P>
	 * This is a shortcut for <CODE>addCell(Cell cell, Point location)</CODE>. The
	 * <CODE>Phrase</CODE> will be converted to a <CODE>Cell</CODE>.
	 *
	 * @param content a <CODE>Phrase</CODE>
	 ...
	 * Adds a <CODE>Cell</CODE> to the <CODE>Table</CODE>.
	 * <P>
	 * This is a shortcut for <CODE>addCell(Cell cell)</CODE>. The <CODE>String</CODE> will be
	 * converted to a <CODE>Cell</CODE>.
	 *
	 ...
	 * <P>
	 * This is a shortcut for <CODE>addCell(Cell cell, Point location)</CODE>. The
	 * <CODE>String</CODE> will be converted to a <CODE>Cell</CODE>.
	 *
	 * @param content a <CODE>String</CODE>
	 * @param location a <CODE>Point</CODE>
	 * @throws BadElementException this should never happen
	...
	/**
	 * To put a table within the existing table at the current position generateTable will of course
	...
	private void mergeInsertedTables() {
		...
		int[] lDummyWidths = new int[columns];     // to keep track in how many new cols this one
												     // will be split
		float[][] lDummyColumnWidths = new float[columns][]; // bugfix Tony Copping
		int[] lDummyHeights = new int[rows.size()]; // to keep track in how many new rows this one
													 // will be split
		...
		if (lTotalColumns != columns || lTotalRows != rows.size() || isTable)    // NO ADJUSTMENT
		{
			...
			for (int tel = 0; tel < widths.length; tel++) {
				if (lDummyWidths[tel] != 1) {
					...
					for (int tel2 = 0; tel2 < lDummyWidths[tel]; tel2++) {
						...
						lNewWidths[lDummy] = widths[tel] * lDummyColumnWidths[tel][tel2] / 100f; // bugfix
																								 // Tony
																								 // Copping
						lDummy++;
					}
			...
			int lDummyRow = 0, lDummyColumn = 0;        // to remember where we are in the new,
												        // larger table
			...
			for (i = 0; i < rows.size(); i++) {
				...
				lNewMaxRows = 1;
				for (j = 0; j < columns; j++) {
					if (Table.class.isInstance(((Row) rows.get(i)).getCell(j)))       // copy values
																				       // from
																				       // embedded
																				       // table
					{
						...
						// Work out where columns in table table correspond to columns in current
						// table
						...
						for (int k = 0; k < lDummyTable.getDimension().height; k++) {
							for (int l = 0; l < lDummyTable.getDimension().width; l++) {
								...
								if (lDummyElement != null) {
									...
									((Row) newRows.get(k + lDummyRow)).addElement(lDummyElement, col);  // use
																										  // addElement
																										  // to
																										  // set
																										  // reserved
					...
					} else        // copy others values
					{
						...
						if (Cell.class.isInstance(aElement)) {
							...
							// most likely this cell covers a larger area because of the row/cols
							// splits : define not-to-be-filled cells
	...
	private boolean isValidLocation(Cell aCell, Point aLocation) {
		...
		if (aLocation.x < rows.size())        // if false : new location is already at new,
										        // not-yet-created area so no check
		{
	 ...
	 * <P>
	 * This method translates the widths expressed in percentages into the x-coordinate of the
	 * borders of the columns on a real document.
	 *
	 * @param left this is the position of the first border at the left (cellpadding not included)
	 * @param totalWidth this is the space between the first border at the left and the last border
	 *            at the right (cellpadding not included)
